...
<div class="page-header">
    <div>
        <div class="page-title">Edit Jadwal Peminjaman</div>
        <div class="page-subtitle">Perbarui jadwal #{{ $schedule->id }}</div>
    </div>
...
<div style="max-width:600px;">
    <div class="card">
        <div class="card-header">
            <div class="card-title">Detail Jadwal</div>
            <span style="font-size:12px;color:var(--text-muted);">ID #{{ $schedule->id }}</span>
        </div>
        <div class="card-body">
            <form method="POST" action="{{ route('admin.schedules.update', $schedule) }}">
                ...
                <div class="form-grid">
                    ...
                    <div class="form-group">
                        ...
                        <select id="inventory_id" name="inventory_id" class="form-control {{ $errors->has('inventory_id') ? 'is-invalid' : '' }}">
                            ...
                            @foreach($inventories as $inv)
                                <option value="{{ $inv->id }}" {{ old('inventory_id', $schedule->inventory_id) == $inv->id ? 'selected' : '' }}>
                                    {{ $inv->name }} — {{ $inv->type }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-grid form-grid-2">
                        <div class="form-group">
                            ...
                            <input id="start_time" type="datetime-local" name="start_time" class="form-control {{ $errors->has('start_time') ? 'is-invalid' : '' }}" value="{{ old('start_time', $schedule->start_time->format('Y-m-d\TH:i')) }}">
                            @error('start_time') <div class="invalid-feedback">{{ $message }}</div> @enderror
                        </div>
                        <div class="form-group">
                            <label class="form-label" for="end_time">Waktu Selesai <span style="color:var(--danger)">*</span></label>
                            <input id="end_time" type="datetime-local" name="end_time" class="form-control {{ $errors->has('end_time') ? 'is-invalid' : '' }}" value="{{ old('end_time', $schedule->end_time->format('Y-m-d\TH:i')) }}">
                ...
                <div style="display:flex;gap:8px;margin-top:24px;padding-top:20px;border-top:1px solid var(--border);">
                    ...
                    <a href="{{ route('admin.schedules.show', $schedule) }}" class="btn btn-secondary">Batal</a>
                </div>
            </form>
        </div>
    </div>
